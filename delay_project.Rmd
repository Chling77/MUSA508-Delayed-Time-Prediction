...
grid.arrange(
  ggplot() +
    ...
    scale_colour_viridis(direction = -1,discrete = FALSE, option = "D") +
    labs(title = "Station Delay For One Month in Oct, 2019") + mapTheme(),
   ggplot() +
    geom_sf(data = NJTracts, color = 'grey') + 
    geom_sf(data = temp, aes(color = mean_pe )) +
    scale_colour_viridis(direction = -1,discrete = FALSE, option = "D") +
    labs(title = "Station Delay For One Month in Oct, 2019") + mapTheme(),nrow = 1 )
```
...
grid.arrange(
  ggplot() +
    ...
    geom_sf(data = temp, aes(color = mean_pe)) +
    scale_colour_viridis(direction = -1,discrete = FALSE, option = "D") +
    labs(title = "Mean Percent Error Comaprison in Weekend") +
    facet_wrap(~weekend)+mapTheme(),
  ggplot() +
    geom_sf(data = NJTracts, color = 'grey') + 
    geom_sf(data = temp, aes(color = mean_ae)) +
...
grid.arrange(
  ggplot() +
    geom_sf(data = NJTracts, color = 'grey') + 
    geom_sf(data = temp,aes(color = mean_pe, geometry = geometry))+
    scale_colour_viridis(direction = -1,discrete = FALSE, option = "D") +
    labs(title = "Mean Percent Error in Line")+mapTheme(),
ggplot() +
    geom_sf(data = NJTracts, color = 'grey') + 
...
reg.cv <- 
  train(delay_minutes ~ from + to + hour + weekend + Temperature + Precipitation + Wind_Speed + lagHour + lag2Hours +lag3Hours +lag12Hours + lag1day + lagsstation + lags2station + lags3station+ lags4station+ lags5station+lags6station+ stop_sequence + line + to_inter, delay.Train, 

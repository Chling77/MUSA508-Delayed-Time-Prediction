...
#Data Wrangling
## Data Source
*[NJ Transit Delay Data](https://www.kaggle.com/datasets/pranavbadami/nj-transit-amtrak-nec-performance) ----
*[NJ Rail Station & Line Data](https://njogis-newjersey.opendata.arcgis.com) ---- 
*[Weather Data]() ----
...
reg.30 <- 
  lm(delay_minutes ~  hour + weekend + Temperature + Precipitation + Wind_Speed + lag45min 
...
reg.90 <- 
  lm(delay_minutes ~  from + to + hour + weekend + Temperature + Precipitation + Wind_Speed
    + lag1h30min + lag1h45min + lag2h + lag2h15min + lag2h30min + lag2h45min + lag3h + lags5station+lags6station + stop_sequence + line + to_inter + from_inter, 
     data=delay.Train)
```
...
grid.arrange(
  delay.Test_5%>%
  dplyr::select(interval60, from, delay_minutes, pre_5) %>%
  gather(Variable, Value, -interval60, -from) %>%
    group_by(Variable, interval60) %>%
    summarize(Value = mean(Value))%>%
  ...
  gather(Variable, Value, -interval60, -from) %>%
    ...
    geom_line(size = 0.9)+
     ...
     theme_minimal(),
  delay.Test_7%>%
  dplyr::select(interval60, from, delay_minutes, pre_7) %>%
  gather(Variable, Value, -interval60, -from) %>%
    group_by(Variable, interval60) %>%
    summarize(Value = mean(Value))%>%
    ggplot(aes(interval60, Value, colour=Variable)) + 
...
ggplot() +
    geom_sf(data = NJTracts, color = 'grey') + 
    geom_sf(data = temp4, aes(color = mean_ae,size = mean_ae)) +
    scale_colour_viridis(direction = -1,discrete = FALSE, option = "D") +
...
```{r error weekend map comparison}
temp <- delay.Test_6 %>% 
...
grid.arrange(
  ggplot() +
    geom_sf(data = NJTracts, color = 'grey') + 
    geom_sf(data = temp2,aes(color = mean_ae, geometry = geometry))+
    scale_colour_viridis(direction = -1,discrete = FALSE, option = "D") +
  ...
  ggplot() +
    ...
    scale_colour_viridis(direction = -1,discrete = FALSE, option = "D") +
    labs(title = "Mean Abosulte Error in Line")+mapTheme(),nrow= 1)
```

```{r over-under-predict}
grid.arrange( ggplot()+
  geom_point(data = delay.Test_5,aes(x= delay_minutes, y = pre_5))+
    geom_smooth(data = delay.Test_5,aes(x= delay_minutes, y= pre_5), method = "lm", se = FALSE, color = "red")+
  ...
  plotTheme(),
  ggplot()+
  geom_point(data = delay.Test_6,aes(x= delay_minutes, y = pre_6))+
    geom_smooth(data = delay.Test_6,aes(x= delay_minutes, y= pre_6), method = "lm", se = FALSE, color = "red")+
    geom_abline(slope = 1, intercept = 0)+
  facet_grid(~weekend)+
  labs(title="Observed vs Predicted",
       x="Observed delay minutes", 
...
reg.cv.30 <- 
  train(delay_minutes ~ hour + weekend + Temperature + Precipitation + Wind_Speed + lag45min + lag1h + lag1h15min + lag1h30min + lag1h45min + lag2h + lag2h15min + lag2h30min + lag2h45min + lag3h 
+ lags3station+ lags4station+ lags5station+lags6station+ stop_sequence + line + to_inter + from_inter, delay.Train, 
        method = "lm", trControl = fitControl, na.action = na.pass)

reg.cv.60 <- 
  train(delay_minutes ~ from + to + hour + weekend + Temperature + Precipitation + Wind_Speed + lag1h + lag1h15min + lag1h30min + lag1h45min + lag2h + lag2h15min + lag2h30min + lag2h45min + lag3h + lags5station + lags6station + stop_sequence + line + to_inter + from_inter, delay.Train, 
        method = "lm", trControl = fitControl, na.action = na.pass)

reg.cv.90 <- 
  train(delay_minutes ~ from + to + hour + weekend + Temperature + Precipitation + Wind_Speed
    + lag1h30min + lag1h45min + lag2h + lag2h15min + lag2h30min + lag2h45min + lag3h + lags5station+lags6station + stop_sequence + line + to_inter + from_inter, delay.Train, 
        method = "lm", trControl = fitControl, na.action = na.pass)
```

```{r mod-genral evaluate}

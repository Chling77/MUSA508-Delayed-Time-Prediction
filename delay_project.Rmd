...
```{r map visualization}
# add a delay time about big city/small city
...
selected_columns <- merged_dataset_station[, c("delay_minutes", "lagHour","lag2Hours","lag3Hours","lag4Hours","lag12Hours","lag1day","week")]
cor_delay_all_time <- cor(selected_columns, use = "complete.obs")["delay_minutes", -1]%>%
  as.data.frame()%>%rename(cor_score = '.')

plotData.lag_time <-
  filter(as.data.frame(merged_dataset_station), week == 42) %>%
  dplyr::select(lagHour,lag2Hours,lag3Hours,lag4Hours,lag12Hours,lag1day, delay_minutes) %>%
...
ggplot(plotData.lag_time, aes(Value,delay_minutes))+
  ...
  labs(title = "Delay minute from previous hours as a function of spatial lags",
...
selected_columns <- merged_dataset_station[, c("delay_minutes", "lagsstation", "lags2station", "lags3station", "lags4station", "lags5station", "lags6station","week")]
...
plotData.lag_station <-
  ...
  gather(Variable, Value, -delay_minutes) %>%
  mutate(Variable = fct_relevel(Variable, "lagsstation", "lags2station", "lags3station", "lags4station", "lags5station", "lags6station"))
...
dplyr::select(lagHour,lag2Hours,lag3Hours,lag4Hours,lag12Hours,lag1day,lagsstation,lags2station,distance,Temperature,Precipitation,Wind_Speed)
